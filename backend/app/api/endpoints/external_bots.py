...
@router.post("/connect", response_model=BotResponse, status_code=status.HTTP_201_CREATED)
async def connect_external_bot(
    ...
    existing_bot = db.query(models_bot.Bot).filter(
    ...
    db_bot = crud_bot.create_bot(db, bot_data=db_bot_data, tenant_id=current_user.tenant_id)
